...
class MetaMysqlRepository:
    ...
    async def save_column_metric(self, column_metrics:list[ColumnMetric]):
        self.session.add_all([ColumnMetricMapper.to_model(column_metric) for column_metric in column_metrics])

    async def save_metric_ifos(self, metric_infos:list[MetricInfo]):
        self.session.add_all([MetricInfoMapper.to_model(metric_info) for metric_info in metric_infos])

    async def get_column_info_by_id(self, relevant_column:str)->ColumnInfo|None:
